...
# Look for model in med2vec folder
...
def collate(batch):
    ...
    return pad_sequence(seqs, batch_first=True), pad_sequence(deltas, batch_first=True), torch.tensor(y_c), torch.tensor(y_r)
...
class MambaModel(nn.Module):
    ...
    def forward(self, x, t):
        ...
        out = self.mamba(emb + t_emb)
        final = out[:, -1, :]
        return self.head(final), self.reg(final)
